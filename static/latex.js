...
function courseworkBlock(resume) {
  const rows = (resume.coursework || []).filter((s) => s.category || s.items)
  if (!rows.length) return ""
  const lines = rows.map((s) =>
    s.category ? `\\textbf{${formatLatex(s.category)}}{: ${formatLatex(s.items)}}` : formatLatex(s.items)
  )
  return `\\section{Relevant Coursework}\n\\begin{itemize}[leftmargin=0.15in, label={}]\n\\small{\\item{\n${lines.join(" \\\\\n")}\n}}\n\\end{itemize}`
}

function certsBlock(resume) {
  const items = (resume.onlineCerts || []).filter((c) => c.name || c.issuer)
  if (!items.length) return ""
  const body = items.map((item) => {
    const name = item.link
      ? `\\textbf{${href(item.link, item.name || item.link)}}`
      : `\\textbf{${formatLatex(item.name)}}`
    const issuer = item.issuer ? ` $|$ \\emph{${formatLatex(item.issuer)}}` : ""
    return `\\resumeProjectHeading{${name}${issuer}}{${formatLatex(item.dates)}}\n${bullets(item.details)}`
  })
  return `\\section{Online Certifications}\n\\resumeSubHeadingListStart\n${body.join("\n")}\n\\resumeSubHeadingListEnd`
}
...
\newcommand{\resumeItemListStart}{\nopagebreak\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt, label=\textbullet]}
...
function contentBlocks(resume, forceNarrow = false) {
  return {
    ...
    coursework: () => courseworkBlock(resume),
    onlineCerts: () => certsBlock(resume),
  }
}
