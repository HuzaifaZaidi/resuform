...
function isColumnsMarker(line) {
  const t = line.trim().toLowerCase().replace(/\s+/g, "")
  return t === "columns" || t === "twocolumns" || t === "twocolumn" || t === "layout:columns" || t === "layoutcolumns"
}
...
function parseSkillsBlock(lines) {
  ...
  return {
    layout: isColumnsMarker(first) ? "columns" : "list",
    items: parseSkills(lines),
  }
}
  ...
  for (const block of blocks) {
    ...
    switch (block.name) {
      ...
      case "projects":
        ...
        break
      case "skills": {
        const parsed = parseSkillsBlock(body)
        resume.skills = parsed.items
