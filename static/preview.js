...
import { proofPageStyle } from "./layout.js"
...
function photoFrame(resume) {
  if (resume.photo && String(resume.photo).startsWith("data:image")) {
    return `<img class="photo-frame" src="${esc(resume.photo)}" alt="Candidate photo" />`
  }
  return `<div class="photo-frame empty">Add photo in Fields</div>`
}

function masthead(resume, template = "classic") {
  const items = [
  ...
  if (template === "modern_photo") {
    return `<header class="masthead with-photo">
...
function roleLine(item) {
  return [
    item.company ? `<strong>${formatHtml(item.company)}</strong>` : "",
    item.title ? formatHtml(item.title) : "",
    item.location ? formatHtml(item.location) : "",
  ]
    .filter(Boolean)
...
function rolesHtml(items) {
  return (items || [])
    ...
    .map(
      (e) => `<article>
        <div class="row"><span>${roleLine(e)}</span><span>${formatHtml(e.dates)}</span></div>
        ${bullets(e.bullets)}
      </article>`
    )
    .join("")
...
export function renderPreview(resume, template = "classic") {
  ...
  const useTable = resume.educationLayout === "table" && template !== "two_column" && template !== "two_column_photo"
  const edu = useTable
    ? (() => {
  ...
  const twoColPage = template === "two_column" || template === "two_column_photo"
  const leftIds = visible.filter((id) => id === "skills" || id === "education")
  const rightIds = visible.filter((id) => id !== "skills" && id !== "education")
  const skillsInSidebar = twoColPage && leftIds.length && rightIds.length

  const skillItems = (resume.skills || []).filter((s) => s.category || s.items)
  const skillLine = (s) => `<div class="skill"><strong>${formatHtml(s.category)}:</strong> ${formatHtml(s.items)}</div>`
  const useSkillCols = resume.skillsLayout === "columns" && skillItems.length >= 2 && !skillsInSidebar
  const skills = useSkillCols
    ? (() => {
        const mid = Math.ceil(skillItems.length / 2)
        return `<div class="skills-cols"><div>${skillItems.slice(0, mid).map(skillLine).join("")}</div><div>${skillItems.slice(mid).map(skillLine).join("")}</div></div>`
      })()
    : skillItems.map(skillLine).join("")
  ...
  let content
  if (twoColPage) {
    const photo = template === "two_column_photo" ? photoFrame(resume) : ""
    if (leftIds.length && rightIds.length) {
      content = `<div class="two-col"><aside>${photo}${leftIds.map((id) => blocks[id]?.() || "").join("")}</aside><div class="main-col">${rightIds.map((id) => blocks[id]?.() || "").join("")}</div></div>`
    } else {
      content = `${photo}${visible.map((id) => blocks[id]?.() || "").join("")}`
    }
  } else {
    content = visible.map((id) => blocks[id]?.() || "").join("")
  }

  const pageClass = template === "modern_photo" ? "modern modern_photo" : template === "two_column_photo" ? "two_column two_column_photo" : template
  return `<div class="resume-page ${esc(pageClass)} ${resume.page === "a4" ? "a4" : "letter"}" style="${proofPageStyle(resume, template)}">
    ${masthead(resume, template)}
    ${content}
  </div>`
}
